...
from .mask_predictor_dev import SimpleDecoding
from .backbone_dev import MultiModalSwinTransformer
...
def lavt(pretrained='', args=None):
    if args is None:
        class Args:
            def __init__(self):
                self.swin_type = 'base'  # tiny, small, base, large 중 선택
                self.window12 = True
                self.mha = None  # 멀티 헤드 어텐션 값
                self.fusion_drop = 0.1  # 드롭아웃 확률
