...
class MultiModalSwinTransformer(nn.Module):
    ...
    def forward(self, x, l, l_mask):
        # x : 스펙토그램, l은 CLAP 768임베딩, l_mask는 마스크
        # x => torch.Size([12, 3, 1024, 512]) 예상
        # debug
        print('input x, 스펙토그램의 차원',x.shape)
        print('input l, CLAP 임베딩의의 차원',l.shape)
        print('input l_mask 마스크의 차원',l_mask.shape)

        """Forward function."""
        x = self.patch_embed(x)
        # debug
        print('self.patch_embed 후 input:x 차원',x.sahpe)
