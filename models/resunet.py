...
class ResUNet30_Base(nn.Module, Base):
    ...
    def forward(self, mixtures, film_dict):
        ...
        """
        # debug
        print('mixtures의 차원 : ', mixtures.shape)

        mag, cos_in, sin_in = self.wav_to_spectrogram_phase(mixtures)
        x = mag

        # debug
        print('스펙토그램 x(mag)의 차원 : ', x.shape)
        print('cos_in의 차원 : ', cos_in.shape)
        print('sin_in의 차원 : ', sin_in.shape)
        ...
        # debug
        print('사이즈 조정 끝난 스펙토그램 x의 차원 : ', x.shape)
        ...
        # debug
        print('1 pool,x : ', x1_pool.shape, x1.shape)
        print('2 pool,x : ', x2_pool.shape, x2.shape)
        print('3 pool,x : ', x3_pool.shape, x3.shape)
        print('4 pool,x : ', x4_pool.shape, x4.shape)
        print('5 pool,x : ', x5_pool.shape, x5.shape)
        print('6 pool,x : ', x6_pool.shape, x6.shape)
        print('center : ', x_center.shape)
        ...
        print('ResUnet을 거친 x의 차원 : ', x.shape)
        ...
        # debug
        print('Recover 거친 x의 차원 : ', x.shape)
        ...
        # debug
        print('오디오로 바꾼 데이터separated_audio의 차원 : ',separated_audio.shape)

        output_dict = {'waveform': separated_audio}

        # debug
        print('output_dict 차원 출력 : ',output_dict)
        return output_dict
...
class ResUNet30(nn.Module):
    ...
    def forward(self, input_dict):
        # debug
        #print('input_dict확인',input_dict)

        mixtures = input_dict['mixture']
        conditions = input_dict['condition']

        # debug
        #print('mixtures, conditions 확인 ',mixtures.shape, conditions.shape)
